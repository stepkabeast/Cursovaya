...
main :: IO ()
-- the first Float is the time, the second Float is the characteristic for the turn
...
main =
  ...
  where
    ...
    progress x y = Pictures [background, earth, grossN1, grossN2, grossN3, sun, sun2, moon, moon2, moonJr, moonJr2]
      where
        ...
        earth
          = Pictures
                [ Translate 0 (-800)
                $ Color (earthColor times) (circleSolid 600)
                ]
    ...
    -- phys update, here we just increase time (x)
